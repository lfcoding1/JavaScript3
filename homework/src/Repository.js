...
class Repository {
  ...
  renderContainer(data){
    async function get0API(url){
      try{
        let index0data = await Util.fetchJSON(index0API)
        await renderIndex0(index0data) 
        } catch(error) {
  ...
  console.log(data);
    ...
    function renderIndex0(index0data){
      console.log(index0data);
      for (let i = 0; i < index0data.length; i++){          
        let Image0Link = Util.createAndAppend('li', contributorsUl, {})
        let contributor0Name = Util.createAndAppend('img', Image0Link, {src: index0data[i].avatar_url, class: 'imageSrc', alt: 'Image Icon of Contributor'});
        let contributor0Link = Util.createAndAppend('a', Image0Link, {text: index0data[i].login, target: "_blank", href: index0data[i].html_url, id: 'link'});
        let contributor0Badge = Util.createAndAppend('li', Image0Link, {text:"Contributions: " + index0data[i].contributions, class: 'badge'});
      } //end for
